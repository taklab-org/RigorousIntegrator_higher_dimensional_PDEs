...
ba_X = wnorm(reshape(chebmag(reshape(intval(ba),n,N1*N2)),N1,N2),nu);
% ba_X_alt = wnorm(permute(sum(abs(intval(ba)),1),[2,3,1]),nu);
%
...
%
mu_m = intval(lambda) - (1-(m(1)*intval(L(1))).^2-(m(2)*intval(L(2))).^2).^2;
%
%
% setting for SH
...
n = n + n_pad;
%
[Phi,~,Y0,Z0,Z1] = rig_2D_SH_variational(ba,m,Nop_coeff,h,lambda,nu_var,L);
r1 = Y0./(1-intval(Z0)-intval(Z1));
Phi = reshape(Phi,n,m1m2,m1m2);
%
nu_var = 1.65;
[Psi,~,Y0,Z0,Z1] = rig_2D_SH_adjoint(ba,m,Nop_coeff,h,lambda,nu_var,L);
