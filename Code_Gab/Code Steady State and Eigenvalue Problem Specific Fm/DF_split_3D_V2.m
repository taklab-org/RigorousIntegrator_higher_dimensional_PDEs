function [D11,D12,D21] = DF_split_3D_V2(a,L,mu_k,Nop_coeff,q,N_Fm_ext,N_Fm)
Nb = length(a);
if length(L) == 2
...
indices_long = reshape(indices_long,size(N_Fm_ext));
indices_long = iso_3D_mat2vec_Fm( indices_long ,  N_Fm_ext);
...
N_Fm_size = size(N_Fm);
N_Fm_pad(1:N_Fm_size(1),1:N_Fm_size(2),1:N_Fm_size(3)) = N_Fm;

N_Fm_delta = iso_3D_mat2vec_Fm( N_Fm_ext - N_Fm_pad,N_Fm_ext) ;
