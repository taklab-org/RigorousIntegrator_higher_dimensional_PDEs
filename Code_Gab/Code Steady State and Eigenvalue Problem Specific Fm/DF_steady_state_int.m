function [DF] = DF_steady_state_int(a,L,mu_k,Nop_coeff,q,N_Fm)
Nb = length(a);
if length(L) == 2
...
for i = 1:numel(mid(a))
    if length(L) == 2
        DF_temp1(:,i) = iso_mat2vec_Fm_int(reshape(DF(:,i),N+1), N_Fm);
    elseif length(L) == 3
...
for i = 1:Nb
    if length(L) == 2
        DF_temp2(i,:) = transpose(iso_mat2vec_Fm_int(reshape(transpose(DF_temp1(i,:)),N+1), N_Fm));
    elseif length(L) == 3
