function [G_0,C,rho,epsilon,delta] = Gershgorin_ring_3D_SH(a,P,mu_k,Nop_coeff,L,r,nu,mu_m,q)
...
for i = 1:numel(mid(a))
    ...
    norm_p_i(i) = norm_1_nu_int(p_i,nu);
    norm_p_i_1(i) = norm_1_nu_int(p_i,1);

    v_i_ext = intval(zeros(3*Na+1));
    v_i_ext(1:Na(1)+1,1:Na(2)+1,1:Na(3)+1) = p_i;
    a2v_i = convapbqcos(a_ext,2,v_i_ext,1);
    a2v_i(1:Na(1)+1,1:Na(2)+1,1:Na(3)+1) = 0;

    max_a2v(i) = norm_1_nu_int(a2v_i,nu);
    a2v = a2v + a2v_i*h(i);

end
Epsilon21 = 3*norm_1_nu_int(a2v,1);

C_2 = max([norm_B_nu_int(P_inv,1,Na),1]);

R = transpose(sum(abs([A;Nap_i_ext]))+ C_2*(6*r*norm_a_nu+3*r^2)*norm_p_i);
B = B + norm_B_nu_int(P_inv,nu,Na)*(6*r*norm_a_nu+3*r^2)*transpose(norm_p_i);
