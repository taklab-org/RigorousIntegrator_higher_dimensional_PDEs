function [r, Y_0,Z_0,Z_1] = Bounds_int(mu_k,Nop_coeff,a_bar,c,h,e_j,nu,DF_nm,L,lambda)
%==========================================================================
%Parameters
%==========================================================================
if ~exist('lambda','var')
...
M = (size(Nop_coeff,2)-1)*(n-1)+1; % Size of the extension(e.g. M = 3n-2 for 2D SH)
    ...
    for j = 0:size(Nop_coeff,1)-1
        if Nop_coeff(j+1,i+1) ~=0
            if j == 0
                ...
                NN = size(a_bar)-1;
                dim = length(NN)-1;
                s = arrayfun(@(k) 0:k, NN(1:end-1), 'UniformOutput', false);
                k = cell(1, dim);
                [k{:}] = ndgrid(s{:});
                K_sq = zeros(size(k{1}));
                for ell = 1:dim
                    K_sq = K_sq + (k{ell}*L(ell)).^j;
                end
                norm_ka = norm_1_nu_int(K_sq.*a_bar,nu);
                Z_1_2 = Z_1_2 +1/(2*n)*(nu+1/nu)*(h*abs(Nop_coeff(i+1))*norm_ka^(i-1));
            end
        end
    end
end
