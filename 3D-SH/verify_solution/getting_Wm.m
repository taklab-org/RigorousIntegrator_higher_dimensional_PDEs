...
alp(1,1,1) = 1;
v1 = nu.^(0:m1-1); v2 = nu.^(0:m2-1); v3 = nu.^(0:m3-1);
w_k0 = ((permute(v3,[3,1,2]).*ones(m1,m2,m3)) .* (v1'*v2)) .* alp;
w0vec = reshape(w_k0,m1m2m3,1);
Wm = max(max(permute(sum(permute(abs(Phi_Psi_value_on_mesh),[2,3,1]).*w0vec,1),[2,3,1])./w0vec,[],2));
% 
%%%%%%%%%%%%     Wm_at_t1 bounds     %%%%%%%%%%%%%%
% 
% Phi values on each mesh
% iPhi_at_t1 = sum(Phi,1) + r_minus*infsup(-1,1);
iPhi = intval(Phi);
